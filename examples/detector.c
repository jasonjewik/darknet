...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            // Create the output YOLOv3 class file name
            strncpy(yolov3_fname, input, 256);
            strtok(yolov3_fname, ".");
            strcat(yolov3_fname, ".txt");
        }
        ...
        active_learning(im, dets, nboxes, thresh, names, alphabet, l.classes, yolov3_fname);
        free_detections(dets, nboxes);
        if (outfile)
        {
